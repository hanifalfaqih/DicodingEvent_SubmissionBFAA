...
import id.hanifalfaqih.dicodingevent_submissionbfaa.databinding.FragmentUpcomingBinding
...
class UpcomingFragment : Fragment() {
    ...
    private fun setupSearchBar() {
        with(binding) {
            searchViewUpcoming.setupWithSearchBar(searchBarUpcoming)
            searchViewUpcoming
                .editText
                .setOnEditorActionListener { textView, _, _ ->
                    val query = textView.text.toString()
                    searchBarUpcoming.setText(query)
                    searchViewUpcoming.hide()
                    if (query.isNotEmpty()) {
